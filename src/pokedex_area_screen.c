...
static void Task_HandlePokedexAreaScreenInput(u8 taskId)
{
    ...
    case 1:
        ...
        else if (JOY_NEW(DPAD_LEFT) || (JOY_NEW(L_BUTTON) && gSaveBlock4Ptr->optionsButtonMode == OPTIONS_BUTTON_MODE_LR))
        {
            gTasks[taskId].data[1] = 1;
            PlaySE(SE_DEX_PAGE);
        }
        else if (JOY_NEW(DPAD_RIGHT) || (JOY_NEW(R_BUTTON) && gSaveBlock4Ptr->optionsButtonMode == OPTIONS_BUTTON_MODE_LR))
        {
